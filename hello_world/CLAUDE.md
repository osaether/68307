...
- **`uart_init`**: identical UART setup to the other two — port B pins (`PBCNT`), RX/TX reset, `UACR=0xE0` (Timer mode, `CTMS=110`), `UBG2=0x03`, 8N1, TX only, all interrupts left masked. See the UART note in `../CLAUDE.md` for why the `0xB0`/`0x04` "fix" was reverted.
