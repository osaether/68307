...
- **`putch`**: if TX buffer empty and TXRDY set, writes directly to `UTB` and sets `UIMR=0x02` (RxRDY only — no TxRDY needed, the byte is already gone); otherwise enqueues to `txbuf` (busy-waits if full) and sets `UIMR=0x03` to keep both RxRDY and TxRDY interrupts enabled
