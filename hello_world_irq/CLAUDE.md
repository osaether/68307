...
- **`uart_init`**: configures port B pins (`PBCNT`), resets RX/TX, sets `UACR=0xB0` (baud set 2, counter mode, `CTMS=011` — the old `0xE0` was the invalid `110` encoding, see Table 8-11), baud rate 115200 (`UBG2=0x04`, i.e. 14.7456 MHz / (32 × 4)), 8N1, enables TX only — interrupts off initially
